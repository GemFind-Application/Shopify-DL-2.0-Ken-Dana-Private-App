...
class ImportController extends Controller
{
    function importApi(Request $request)
    {
        ...
        foreach ($requestData as $value) {
            ...
            if ($value['value'] == 1) {
                $saveImportData =  DB::table('diamondlink_config')->where(['shop' => $value['shopDomain']])->update(['type_1' => "1"]);
    ...
    {
        $importData = DB::table('diamondlink_config')->where(['shop' => $shopDomain])->value('type_1');
        if (!empty($importData)) {
            return $importData;
        } else {
            return 0;
